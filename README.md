...
```
_buttonCard: number - Кнопка карточки.
```
...
```
countCards() - Подсчитывает и отображает количество карточек в корзине.
```
...
Класс предназначенный для отображения и управления формой. Реализует интерфейс "IUserData". Наследуется от "Component<T>".
свойства 
